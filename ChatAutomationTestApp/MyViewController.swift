...
extension MyViewController {
    ...
    func createThread(description: String?, image: String?, invitees: [Invitee]?, metadata: String?, title: String?, type: String?, requestUniqueId: String?,
        ...
        func createNormalThreadWithContactId(cellphoneNumber: String?, email: String?, firstName: String?, lastName: String?) {
            ...
            addContact(cellphoneNumber: cellphoneNumber, email: email, firstName: firstName, lastName: lastName, uniqueId: { _ in }) { (contactModel) in
                if let myContact = contactModel.contacts.first {
                    if let contactId = myContact.id {
            ...
            addContact(cellphoneNumber: cellphoneNumber, email: email, firstName: firstName, lastName: lastName, uniqueId: { _ in }) { (contactModel) in
                if let myContact = contactModel.contacts.first {
                    if let contactId = myContact.cellphoneNumber {
                        ...
                        sendRequest(theDescription: fakeParams.description, theImage: image, theInvitees: [myInvitee], theMetadata: metadata, theTitle: fakeParams.title, theType: threadType, theRequestUniqueId: nil)
